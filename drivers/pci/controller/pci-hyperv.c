...
static void __exit exit_hv_pci_drv(void)
{
	...
	if (hv_pci_swiotlb_pool) {
		...
		/*
		 * Clear the tracking variables unconditionally: on destroy
		 * failure we intentionally leak this pool once (the backing
		 * pages may still be shared with the host), so a hypothetical
		 * re-entry must not try to destroy or free stale pointers.
		 */
...
static int __init hv_pci_swiotlb_alloc_pool(void)
{
	...
	if (IS_ERR(hv_pci_swiotlb_pool)) {
		long err = PTR_ERR(hv_pci_swiotlb_pool);

		pr_err("hv_pci: failed to create swiotlb pool: %ld\n", err);
		hv_pci_swiotlb_pool = NULL;
		/*
		 * -EIO from swiotlb_create_pool() means decrypt+rollback both
		 * failed and the backing range may be partially shared with
		 * the host.  Returning it to the buddy allocator on a
		 * confidential guest would hand out shared pages, so leak it.
		 */
		if (err == -EIO)
			pr_err("hv_pci: leaking %zu-byte backing range at %pa (encryption state unknown)\n",
			       hv_pci_swiotlb_size, &hv_pci_swiotlb_base);
		else
