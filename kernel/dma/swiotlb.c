 ...
 * @mem:	The io_tlb_mem returned by swiotlb_create_pool().  Must NOT be
 *		&io_tlb_default_mem or a transient pool from the
 *		CONFIG_SWIOTLB_DYNAMIC fast path.
 *
 ...
 * before calling.  A synchronize_rcu() inside this function then waits for
 * any straggling RCU readers of @mem's internal pool list to drain before
 * the backing structures are freed.
 *
 * Must be called from process context (set_memory_encrypted(),
 * debugfs_remove_recursive() and synchronize_rcu() may all sleep).
 *
 * Return:
 * * %0 on success.
 * * %-EINVAL if @mem is the global default pool (catches accidental misuse).
...
int swiotlb_destroy_pool(struct io_tlb_mem *mem)
{
	...
	size_t bytes = pool->end - pool->start;
	int ret;

	if (WARN_ON_ONCE(mem == &io_tlb_default_mem))
		return -EINVAL;
	...
	/*
	 * add_mem_pool() linked pool->node into mem->pools via list_add_rcu().
	 * The caller has guaranteed no device still references @mem, so no new
	 * reader can enter mem->pools; wait for any pre-existing readers to
	 * leave before freeing @mem (which contains the list head).
	 */
	synchronize_rcu();
